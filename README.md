...
# 4.git 错误与解决方案：
    4.1 在提交git的时候提示“The local repository is out of date”，解决方案：cd到当前项目目录，输入：git pull --rebase origin master，然后git push即可。
    4.2 error: Cannot pull with rebase: You have unstaged changes. error: Additionally, your index contains uncommitted   changes.，解决方案：
    先执行git stash 
    再执行git pull –rebase 
    最后再执行git stash pop
# 下一节课：19.MVVM设计模式
